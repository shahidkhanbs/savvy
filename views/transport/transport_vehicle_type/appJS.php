<script>
    ...
    $(document).on('click', '#save_stop', function(e) {
        ...
        $.ajax({
            data: data,
            type: "post",
            url: base_url + "savvy1/TransportVehicleType/store",
            success: function(data) {
            	$('#add_stop_modal').modal('hide'); 
            	$('#msgs').html("<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> Transport Vehicle Type Added Successfully</div>");
    ...
    function deleteVehicleType(id, status) {
        confirmDelete('Do you want to DELETE this record?').then(function() {
            var base_url = '<?=base_url(); ?>';
            $.ajax({
                type: 'POST',
                url: base_url + "savvy1/TransportVehicleType/toggle",
                data: {
                    id: id,
                    status: status
                },
    ...
    function activeVehicleType(id, status) {
    ...
    function editVehicleType(id) {
            ...
            $.ajax({
                type: 'POST',
                url: base_url + "savvy1/TransportVehicleType/edit",
                data: {
                    id: id
                },
                dataType:"json", 
    ...
    $(document).on('click', '#edit_stop', function(e) {
        ...
        $.ajax({
            data: data,
            type: "post",
            url: base_url + "savvy1/TransportVehicleType/update",
            success: function(data) {
            	$('#edit_stop_modal').modal('hide'); 
            	$('#msgs').html("<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> Transport Vehicle Type Updated Successfully</div>");
